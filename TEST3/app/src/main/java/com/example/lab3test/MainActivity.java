            ...
            public void onClick(View v) {
                if (ed_name.length() < 1)
                    ...
                    if (btn_scissor.isChecked())
                        ...
                        tv_mmora.setText("我方出拳\n布");
                    int compurter = (int) (Math.random() * 3);
                    if (compurter == 0)
                        tv_cmora.setText("電腦出拳\n剪刀");
                    else if (compurter == 1)
                        tv_cmora.setText("點腦出拳\n石頭");
                    else
                        tv_cmora.setText("點腦出拳\n布");
                    if ((btn_scissor.isChecked() && compurter == 2) ||
                            (btn_stone.isChecked() && compurter == 0) ||
                            (btn_paper.isChecked() && compurter == 1)) {
                        tv_winner.setText("勝利者\n" + ed_name.getText().toString());
                        tv_text.setText("恭喜你獲勝了！！！");
                    } else if ((btn_scissor.isChecked() && compurter == 1) ||
                            (btn_stone.isChecked() && compurter == 2) ||
                            (btn_paper.isChecked() && compurter == 0)) {
                        tv_winner.setText("勝利者\n電腦");
